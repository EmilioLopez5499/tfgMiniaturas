...
function calcularPresupuesto(descripcion: string): Presupuesto {
  const texto = normalizar(descripcion);
  const desglose: DesglosePrecio[] = [];
...
function mostrarPresupuesto(presupuesto: Presupuesto): void {
  ...
  if (!seccion || !desglose || !total) return;
...
async function confirmarPedido(presupuesto: Presupuesto): Promise<void> {
  const token = localStorage.getItem('token');
  const usuarioGuardado = localStorage.getItem('usuario');
  const mensaje = document.getElementById('pedido-mensaje');
  const figura = (document.getElementById('pedido-figura') as HTMLInputElement).value;
  const descripcion = (document.getElementById('pedido-descripcion') as HTMLTextAreaElement).value;

  if (!token || !usuarioGuardado || !mensaje) return;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let presupuestoActual: Presupuesto | null = null;
  ...
  if (btnConfirmar) {
    btnConfirmar.addEventListener('click', () => {
      if (presupuestoActual) confirmarPedido(presupuestoActual);
    });
  }
